...
1. **脑洞任务库规模**：首批提供 10 个还是 20 个脑洞任务？
2. **传说任务解锁通知**：解锁新传说任务时，是否推送实时通知（如 toast）还是仅在下一次加载任务板时显示？
3. **鹤屋商店商品数据**：商店商品列表是硬编码在前端，还是从后端接口获取？
4. **任务分享文案**：分享图上的固定文案（如「我刚刚完成了SOS团的XXX任务！」）是否需要可配置？
